...
{
	class IncidentWorker_SleepParalysis : IncidentWorker
	{
		...
		public List<Pawn> ApplyToPawns(IEnumerable<Pawn> pawns)
		{
			...
			foreach (Pawn pawn in pawns)
			{
				if (!pawn.Awake())
				{
					...
					pawn.health.AddHediff(HediffDefOf.Dagoth_SleepParalysis).Severity = 1f;
				}
			}
		...
		{
			return ("DagothDream_SleepParalysis").Translate();
		}
	}
}
