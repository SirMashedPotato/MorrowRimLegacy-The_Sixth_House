...
{
	class IncidentWorker_SleepersAwake : IncidentWorker
	{
		...
		public List<Pawn> ApplyToPawns(IEnumerable<Pawn> pawns)
		{
			...
			if (list.Any<Pawn>())
			{
				LordMaker.MakeNewLord(faction, new LordJob_ExitMapBest(Verse.AI.LocomotionUrgency.Walk, false, true), list.ElementAt(0).Map, list);
			}
			return list;
		}
		...
		{
			return ("DagothDream_Awakening_Peaceful").Translate();
		}
	}
}
